 ...
 */
export function buildIdeaAccess(
  source: IdeaAccessSource,
): IdeaAccess {
  const isAvailable =
    source.isOwner && !source.isDeleted;

  const hasAdvancedAccess =
    isAvailable && source.isUnlocked;

  const canDirectUnlock =
    isAvailable &&
    !source.isUnlocked &&
    (source.supportsDirectUnlock ?? true);
 ...
 */
export function canViewAdvancedIdeaContent(
  source: IdeaAccessSource,
): boolean {
  return buildIdeaAccess(source)
 ...
 */
export function canPublishIdea(
  source: IdeaAccessSource,
): boolean {

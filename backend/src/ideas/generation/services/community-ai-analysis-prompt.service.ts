...
export class CommunityAiAnalysisPromptService {
  ...
  private buildEvidenceTriageSystemInstruction(): string {
    ...
    return [
      ...
      'STRICT FAMILY SELF-CONSISTENCY: problemFamily must be directly entailed by observedProblem from THIS evidence row. Never copy, paraphrase, or broaden the requester problem into problemFamily when observedProblem only supports a narrower or different issue. If the observedProblem cannot naturally be summarized by the proposed family, use CONTEXT_ONLY or a narrower family.',
      'For a single documentary SUPPORTING_SIGNAL, be especially conservative: do not select a requester-sized family unless the documented finding itself names the same operational friction. A broad security, market, product, or tooling complaint is not proof of a specific factory/equipment failure distinction merely because the request asks about it.',
      ...
      'After classifying all items, choose selectedProblemFamily as the strongest neutral OBSERVED_PROBLEM family supported by DIRECT_PROBLEM/SUPPORTING_SIGNAL items in this supplied corpus. Prefer direct evidence first; otherwise prefer independent-source diversity and repeated support before confidence or specificity. A single-source/single-item niche family must not beat a broader coherent family supported by multiple trusted rows or multiple independent sources merely because the niche row has higher confidence. In discovery mode, choose the most actionable recurring operational family that the supplied evidence itself supports; do not broaden beyond the observed rows. selectedEvidenceIds must contain only trusted rows carrying that exact normalized family. If none exists, return empty selectedProblemFamily and [].',

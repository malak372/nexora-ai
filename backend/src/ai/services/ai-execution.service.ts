...
export class AiExecutionService {
  ...
  constructor(
    ...
    private readonly modelRoutingService:
      AiModelRoutingService,
    private readonly modelHealthService:
      AiModelHealthService,
    private readonly timeoutService:
      AiTimeoutService,
    private readonly externalLogService:
      ExternalAiLogService,
    private readonly structuredOutputService:
      AiStructuredOutputService,
    private readonly responseRepairService:
      AiResponseRepairService,
    configService: ConfigService,
  ) {
    this.timeoutMs =
      this.resolvePositiveIntegerConfig(
        configService,
    ...
    this.maxRetriesPerModel =
      this.resolveNonNegativeIntegerConfig(
        configService,
  ...
  async execute(
    ...
    const models =
      await this.modelRoutingService
      ...
      for (
        ...
        try {
          ...
          const invalidOutputError =
            this.createStructuredOutputError(
              validation.failure,
            );
          ...
          if (
            MAX_AI_STRUCTURED_OUTPUT_REPAIRS > 0
          ) {
            globalAttemptNumber += 1;

            const repairAttemptNumber =
              globalAttemptNumber;

            const repairResult =
              await this.executeRepairAttempt(
                provider,
                model,
                input,
                providerResult.text,
    ...
    const operations: Promise<unknown>[] = [
      this.externalLogService.create({
        ...
        ideaId:
          input.ideaId,
        requestId:
          result.providerResult.requestId,
        endpoint:
          AI_TEXT_GENERATION_ENDPOINT,
        isSuccess:
          true,
  ...
  private buildExecutionResult(
    ...
    return {
      ...
      finishReason:
        result.providerResult.finishReason,
      fallbackUsed,
      attemptCount,
    };
  }

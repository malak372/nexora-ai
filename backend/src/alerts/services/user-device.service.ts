...
export class UserDeviceService {
   ...
   */
  getActiveDevicesForPush(userId: string): Promise<ActivePushDeviceRecord[]> {
    return this.prisma.userDevice.findMany({
      where: {
        userId,
        revokedAt: null,
  ...
  async revokeDevice(userId: string, deviceId: string): Promise<void> {
    ...
    if (result.count === 0) {
      ...
      throw new NotFoundException('Active user device was not found.');
    }
   ...
   */
  async revokeToken(userId: string, fcmToken: string): Promise<boolean> {
    ...
    if (wasRevoked) {
      this.logger.log(`Push-notification token revoked for user ${userId}.`);
    }
   ...
   */
  private normalizeDeviceIds(deviceIds: readonly string[]): string[] {
    return [
      ...new Set(
        deviceIds

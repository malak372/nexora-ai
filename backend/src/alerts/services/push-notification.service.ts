...
export class PushNotificationService {
  ...
  async sendToUser(
    ...
    try {
      const devices = await this.userDeviceService.getActiveDevicesForPush(
        input.userId,
      );
      ...
      const deviceBatches = this.chunk(devices, FCM_MULTICAST_BATCH_SIZE);
      ...
      const revokedTokenCount = await this.userDeviceService.revokeDevices(
        input.userId,
        [...invalidDeviceIds],
      );
      ...
      this.logDeliveryResult(input.userId, result, Date.now() - startedAt);
  ...
  private async sendDeviceBatch(
    ...
    for (
      ...
      try {
        const response = await this.sendBatch(pendingDevices, input);
        ...
        if (
          ...
          await this.delay(PUSH_NOTIFICATION_RETRY_DELAY_MS * attempt);
        }
      } catch (error: unknown) {
  ...
  private sendBatch(
    ...
    return this.firebaseService.getMessaging().sendEachForMulticast(message);
  }
  ...
  private analyzeBatchResponse(
    ...
    response.responses.forEach((sendResponse: SendResponse, index: number) => {
      ...
      if (errorCode && RETRYABLE_FCM_ERROR_CODES.has(errorCode)) {
        retryableDevices.push(device);
        return;
      }

      this.logger.warn(
        `Firebase permanently rejected device ${device.id}${
          errorCode ? ` with error ${errorCode}` : ''
        }.`,
      );
    });

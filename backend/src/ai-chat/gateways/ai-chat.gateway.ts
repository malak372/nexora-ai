    ...
    OnGatewayDisconnect<AuthenticatedSocket>
{
  ...
  private createStreamObserver(
    ...
    return {
      ...
      onChunk: (chunk) => {
        this.server.to(room).emit(AI_CHAT_SERVER_EVENTS.MESSAGE_CHUNK, chunk);
      },
      onCompleted: (message) => {
        this.server.to(room).emit(AI_CHAT_SERVER_EVENTS.MESSAGE_COMPLETED, {
          sessionId,
          message,
        });
      },
      onFailed: (message) => {
        this.server.to(room).emit(AI_CHAT_SERVER_EVENTS.MESSAGE_FAILED, {
          sessionId,
          message,
        });
      },

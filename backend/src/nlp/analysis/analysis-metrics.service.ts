...
export class AnalysisMetricsService {
    ...
    calculate(input: AnalysisMetricsInput): AnalysisQualityMetrics {
        ...
        const totalCollectedTexts =
            this.calculateTotalCollectedTexts(input);

        const extractedResultsCount =
            this.calculateExtractedResultsCount(input);
        ...
        const averageTextConfidence =
            this.calculateAverageTextConfidence(input);
     ...
     */
    private calculateTotalCollectedTexts(
     ...
     */
    private calculateExtractedResultsCount(
        input: AnalysisMetricsInput,
    ): number {
        return (
    ...
    private calculateEvidenceBackedResultsCount(
        ...
        const recurringProblemsWithEvidence =
            input.recurringProblems.filter((problem) =>
                this.hasEvidence(problem.evidenceSamples),
            ).length;

        const extractedNeedsWithEvidence =
            input.extractedNeeds.filter((need) =>
                this.hasEvidence(need.evidenceSamples),
            ).length;

        const featureRequestsWithEvidence =
            input.featureRequests.filter((request) =>
                this.hasEvidence(request.evidenceSamples),
            ).length;

        const opportunitiesWithEvidence =
            input.opportunities.filter((opportunity) =>
                this.hasEvidence(opportunity.evidenceSamples),
            ).length;
     ...
     */
    private calculateAverageTextConfidence(
        input: AnalysisMetricsInput,
    ): number {
        ...
        return this.round(
            totalConfidence / input.analyzedTexts.length,
        );
    }
    ...
    private calculateResultDensity(
        ...
        const targetResultsCount =
            totalAnalyzedTexts * TARGET_RESULTS_PER_TEXT;

        return this.round(
            this.clamp(extractedResultsCount / targetResultsCount),
        );
    }
    ...
    private calculateEvidenceCoverage(
        ...
        return this.round(
            this.clamp(
                evidenceBackedResultsCount / evidenceEligibleResultsCount,
            ),
        );
    }
    ...
    private calculateDataRetentionRate(
        ...
        return this.round(
            this.clamp(totalAnalyzedTexts / totalCollectedTexts),
        );
    }
    ...
    private calculateLexicalCoverage(
        ...
        return this.round(
            this.clamp(textsWithLexiconMatches / totalAnalyzedTexts),
        );
    }
    ...
    private calculateOverallConfidence(input: {
        ...
        const confidence =
            input.averageTextConfidence *
            ANALYSIS_CONFIDENCE_WEIGHTS.textConfidence +
            input.resultDensity *
            ANALYSIS_CONFIDENCE_WEIGHTS.resultDensity +
            input.evidenceCoverage *
            ANALYSIS_CONFIDENCE_WEIGHTS.evidenceCoverage +
            input.dataRetentionRate *
            ANALYSIS_CONFIDENCE_WEIGHTS.dataRetentionRate +
            input.lexicalCoverage *
            ANALYSIS_CONFIDENCE_WEIGHTS.lexicalCoverage;
    ...
    private validateInput(input: AnalysisMetricsInput): void {
        ...
        if (
            input.analyzedTexts.length !== input.totalTextsAnalyzed
        ) {
            throw new Error(
                'Analyzed text count must equal totalTextsAnalyzed.',
            );
        }

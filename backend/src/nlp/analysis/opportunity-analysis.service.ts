...
export class OpportunityAnalysisService {
        ...
        for (const problem of problems) {
            ...
            for (const need of relatedNeeds) {
                opportunities.push({
                    ...
                    solutionArea: this.inferSolutionArea(problem.title, need.need, keywords),
    ...
    private inferSolutionArea(
        ...
        const sourceText = [problem, need, ...keywords.slice(0, 5).map((item) => item.keyword)]
            .filter(Boolean)
            .join(' ')
            .toLowerCase();
     ...
     */
    private mergeSimilarOpportunities(opportunities: Opportunity[]): Opportunity[] {
        ...
        for (const opportunity of opportunities) {
            ...
            if (!existing) {
                opportunityMap.set(key, {
                    ...
                    evidenceSamples: this.pickEvidenceSamples(opportunity.evidenceSamples),
                });

                continue;
            }

            existing.score = this.normalizeScore(existing.score + opportunity.score * 0.25);
    ...
    private pickEvidenceSamples(samples: string[]): string[] {
        return [...new Set(samples.map((sample) => sample.trim()).filter(Boolean))].slice(
            0,
            this.maxEvidenceSamples,
        );
    }

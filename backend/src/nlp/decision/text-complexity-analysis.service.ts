...
export class TextComplexityAnalysisService {
    ...
    analyze(
        ...
        const averageTextLength =
            this.calculateAverageTextLength(input);
        ...
        const mixedSentimentRatio =
            this.calculateMixedSentimentRatio(input);

        const lowConfidenceRatio =
            this.calculateLowConfidenceRatio(input);

        const multiTopicRatio =
            this.calculateMultiTopicRatio(input);

        const unmatchedLexiconRatio =
            this.calculateUnmatchedLexiconRatio(input);
    ...
    private calculateAverageTextLength(
        ...
        const totalWords = input.analyzedTexts.reduce(
            (sum, text) =>
                sum + this.tokenize(text.cleanedText).length,
            0,
        );

        return this.round(
            totalWords / input.analyzedTexts.length,
        );
    }
     ...
     */
    private normalizeAverageTextLength(
        averageTextLength: number,
    ): number {
        return this.round(
            this.clamp(
                averageTextLength / COMPLEX_TEXT_WORD_TARGET,
            ),
        );
    }
    ...
    private calculateLowConfidenceRatio(
        ...
        const lowConfidenceTexts = input.analyzedTexts.filter(
            (text) =>
                text.confidence < LOW_TEXT_CONFIDENCE_THRESHOLD,
        ).length;
     ...
     */
    private calculateMultiTopicRatio(
        input: TextComplexityAnalysisInput,
    ): number {
        ...
        const multiTopicTexts = input.analyzedTexts.filter(
            (text) => {
                ...
                return (
                    matchedTopicCount >= MULTI_TOPIC_MINIMUM_COUNT
                );
            },
        ).length;
    ...
    private calculateUnmatchedLexiconRatio(
        ...
        const unmatchedTexts = input.analyzedTexts.filter(
            (text) =>
                !Object.values(text.matchedLexicons).some(
                    (matches) =>
                        this.hasMeaningfulMatches(matches),
                ),
        ).length;
    ...
    private calculateComplexityScore(input: {
        ...
        const complexityScore =
            input.normalizedTextLength *
            TEXT_COMPLEXITY_WEIGHTS.averageTextLength +
            input.negationRatio *
            TEXT_COMPLEXITY_WEIGHTS.negationRatio +
            input.contrastRatio *
            TEXT_COMPLEXITY_WEIGHTS.contrastRatio +
            input.mixedSentimentRatio *
            TEXT_COMPLEXITY_WEIGHTS.mixedSentimentRatio +
            input.lowConfidenceRatio *
            TEXT_COMPLEXITY_WEIGHTS.lowConfidenceRatio +
            input.multiTopicRatio *
            TEXT_COMPLEXITY_WEIGHTS.multiTopicRatio +
    ...
    private containsTokenSequence(
        ...
        if (
            ...
            signalTokens.length > tokens.length
        ) {
            return false;
        }

        const maximumStartIndex =
            tokens.length - signalTokens.length;
    ...
    private calculateRatio(
        ...
        return this.round(
            this.clamp(numerator / denominator),
        );
    }

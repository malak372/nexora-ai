...
export class AnalysisDecisionService {
    ...
    decide(input: AnalysisDecisionInput): AnalysisDecisionResult {
        ...
        const enhancementReasons =
            this.collectAiEnhancementReasons(input);
    ...
    private collectQualityReasons(
        ...
        const { quality: thresholds } =
            ANALYSIS_DECISION_THRESHOLDS;
    ...
    private pushMinimumViolationReason(input: {
        ...
        input.reasons.push(
            this.createReason(
                ...
                this.calculateMinimumThresholdSeverity(
                    input.actual,
                    input.minimum,
                ),
            ),
    ...
    private pushMaximumViolationReason(input: {
        ...
        input.reasons.push(
            this.createReason(
                ...
                this.calculateMaximumThresholdSeverity(
                    input.actual,
                    input.maximum,
                ),
            ),
    ...
    private calculateRuleBasedSuitabilityScore(
        ...
        const weights =
            ANALYSIS_DECISION_THRESHOLDS.suitabilityWeights;

        const inverseComplexity =
            1 - input.complexityMetrics.complexityScore;

        const score =
            input.qualityMetrics.confidence *
            weights.confidence +
            input.qualityMetrics.resultDensity *
            weights.resultDensity +
            input.qualityMetrics.evidenceCoverage *
            weights.evidenceCoverage +
            input.qualityMetrics.dataRetentionRate *
            weights.dataRetentionRate +
            input.qualityMetrics.lexicalCoverage *
            weights.lexicalCoverage +
            inverseComplexity *
            weights.inverseComplexity;
    ...
    private calculateMinimumThresholdSeverity(
        ...
        return this.round(
            this.clamp((minimum - actual) / minimum),
        );
    }
    ...
    private calculateMaximumThresholdSeverity(
        ...
        return this.round(
            this.clamp((actual - maximum) / (1 - maximum)),
        );
    }
    ...
    private validateInput(input: AnalysisDecisionInput): void {
        ...
        this.validateAverageTextLength(
            input.complexityMetrics.averageTextLength,
        );
    }
     ...
     */
    private validateAnalyzedTextCount(
     ...
     */
    private validateNormalizedMetrics(
        input: AnalysisDecisionInput,
    ): void {
        ...
        const hasInvalidNormalizedMetric =
     ...
     */
    private validateAverageTextLength(
        averageTextLength: number,
    ): void {
        if (
            !Number.isFinite(averageTextLength) ||
            averageTextLength < 0
        ) {
            throw new Error(

...
export default function AdminPublicationReportsPage() {
  ...
  const loadSummary = useCallback(async () => {
    ...
    try {
      ...
      // Summary is supporting information; never block the moderation table.
    } finally {
  ...
  const openReport = (report, tab = 'review') => {
    ...
    // Start every moderation pass with a fresh action composer. Previous
    // actions/messages remain visible in the history block and are never
    // silently re-sent when an administrator only changes the report status.

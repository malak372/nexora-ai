...
const RESOURCE_CONFIG = {
  users: {
    ...
    description:
      'Search, inspect, activate or deactivate accounts, trigger password recovery and safely soft-delete users.',
    icon: UsersRound,
    api: adminApi.users,
    searchable: true,
    exportable: true,
...
function selectColumns(rows, section) {
  ...
  const preferredBySection = {
    users: ['name', 'fullName', 'email', 'role', 'status', 'isActive', 'isVerified', 'userType', 'credits', 'createdAt'],
  ...
  return [...preferred, ...remainder].slice(0, section === 'users' ? 7 : 8);
}
...
function normalizeSummary(summary, rows, section) {
  ...
  if (section === 'users') {
    ...
    const premium = rows.filter((item) => String(item?.role || item?.userType || item?.tier || '').toUpperCase().includes('PREMIUM')).length;
    return [
...
function UserSortHeader({ column, sortBy, sortOrder, onSort }) {
  ...
  if (!field) return <th>{toReadableLabel(column)}</th>;
  ...
  return (
    <th aria-sort={active ? (sortOrder === 'asc' ? 'ascending' : 'descending') : 'none'}>
      <button
        ...
        title={`Sort by ${toReadableLabel(column)}`}
      >
...
export default function AdminResourcePage({ section }) {
  ...
  const [userForm, setUserForm] = useState({ fullName: '', userType: 'OTHER', accountStatus: 'NORMAL', freeGenerationLimit: 3, isVerified: false });
  ...
  const changeUserSortField = (event) => {
  ...
  const handleSaveUser = async () => {
    ...
    try {
      await config.api.update(id, {
        ...
        freeGenerationLimit: Number(userForm.freeGenerationLimit),
  ...
  const renderUserActions = (item) => {
    ...
    const id = item?.id || item?.userId;
    if (!id) return null;
    const active = item?.isActive !== false && String(item?.status || '').toUpperCase() !== 'INACTIVE';

    return (
      <div className="admin-row-actions" onClick={(event) => event.stopPropagation()}>
        <button type="button" title="View user" onClick={() => openUserModal(item, 'view')}>
          <Eye size={14} />
        </button>
        {config.api?.update && (
          <button type="button" title="Edit user" onClick={() => openUserModal(item, 'edit')}>
            <Pencil size={14} />
          </button>
        )}
        {config.api?.status && (
          <button
            ...
            disabled={busyId === id}
            onClick={() => runMutation(id, () => config.api.status(id, !active))}
          >
            {active ? <Ban size={14} /> : <UserCheck size={14} />}
          </button>
        )}
        {config.api?.resetPassword && (
          <button
            ...
            disabled={busyId === id}
            onClick={() => runMutation(id, () => config.api.resetPassword(id))}
          >
            <Mail size={14} />
          </button>
        )}
        {config.api?.remove && (
          <button
            ...
            disabled={busyId === id}
            onClick={() => {
              if (window.confirm('Delete this user account? This action may be irreversible.')) {
                  ...
                  {rows.map((item, index) => {
                    ...
                    return (
                      <tr key={rowId} onClick={() => section === 'users' ? openUserModal(item, 'view') : setSelected(item)}>
                        {columns.map((column) => (
                          <td key={`${rowId}-${column}`}><CellValue value={item?.[column]} column={column} /></td>
                        ))}
                ...
                <div className="admin-user-edit__workspace">
                  ...
                  <div className="admin-user-edit__form">
                    ...
                    <label className="admin-user-edit__field">
                      ...
                      <select value={userForm.accountStatus} onChange={(e) => setUserForm((v) => ({ ...v, accountStatus: e.target.value }))}>
                    ...
                    <label className="admin-user-edit__field">
                      ...
                      <input type="number" min="0" max="1000" value={userForm.freeGenerationLimit} onChange={(e) => setUserForm((v) => ({ ...v, freeGenerationLimit: e.target.value }))} />
                    </label>
                  ...
                  <div className="admin-user-edit__actions">
                    ...
                    <button type="button" className="admin-user-edit__save" onClick={handleSaveUser} disabled={savingUser || !userForm.fullName.trim()}>

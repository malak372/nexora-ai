...
export default function AdminIdeasPage() {
  ...
  const load = useCallback(async ({ quiet = false, force = false } = {}) => {
    ...
    // Start the list and summary together. The list remains the critical request,
    // while the metric cards hydrate independently in the background.
    ...
    summaryPromise
      ...
      .catch(() => {
        // Summary is supportive UI. Never make the directory fail because a
        // statistics query is slower than the table query.
      })
      .finally(() => {
  ...
  const openPublicationInsights = async (idea) => {
    ...
    // Open immediately with the row snapshot. The drawer should never wait on
    // the very large full-admin idea payload.
  ...
  const closePublicationInsights = () => {
